...
export function sanitizeString(value: string): string {
	...
	if (username) {
		sanitized = sanitized.replace(
			new RegExp(escapeRegex(username), "g"),
			userHash,
		);
	}
	return sanitized;
...
export async function collectDiagnostics(
	...
	const otherPiExtensions = packages.filter(
		(entry) => !hasMagicContextPackage([entry]),
	);
	...
	return {
		...
		settings: {
			...
			...(settingsParsed.parseError
				? { parseError: settingsParsed.parseError }
				: {}),
