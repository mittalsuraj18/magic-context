...
import { collectDiagnostics } from "../lib/diagnostics-oh-my-pi";
import { bundleIssueReport } from "../lib/logs-oh-my-pi";
import {
    detectOhMyPiBinary,
    getOhMyPiVersion,
    OH_MY_PI_PACKAGE_SOURCE,
    type PiBinaryInfo,
} from "../lib/oh-my-pi-helpers";
import {
...
import { writeOhMyPiSettingsPackage } from "./setup-oh-my-pi";
...
const MIN_OH_MY_PI_VERSION = "0.71.0";
...
interface DoctorDeps {
    ...
    detectOhMyPiBinary: () => PiBinaryInfo | null;
    getOhMyPiVersion: (piPath: string) => string | null;
...
const DEFAULT_DEPS: DoctorDeps = {
    ...
    detectOhMyPiBinary,
    getOhMyPiVersion,
    getLatestNpmVersion: () => getLatestNpmVersion(OH_MY_PI_PACKAGE_NAME),
    probeEmbeddingEndpoint,
    openDatabase,
    isDatabasePersisted,
    closeDatabase,
...
function cacheRoots(): string[] {
    ...
    return [
        ...
        join(cacheHome, "oh-my-pi", "extensions"),
        join(cacheHome, "oh-my-pi", "packages"),
        join(getOhMyPiAgentConfigDir(), "cache", "extensions"),
    ];
}
    ...
    const visit = (path: string, depth: number): void => {
        ...
        if (existsSync(packageJson)) {
            try {
                ...
                if (name === OH_MY_PI_PACKAGE_NAME || path.includes("oh-my-pi-magic-context")) {
                    if (force || !version || version !== self) found.set(path, { path, version });
                }
            } catch {
                if (path.includes("oh-my-pi-magic-context")) found.set(path, { path });
            }
        }
...
async function runHealthChecks(options: {
    ...
    const pi = options.deps.detectOhMyPiBinary();
    if (!pi) {
        add(results, "fail", "Pi binary not found on PATH or at ~/.omp/bin/pi");
    } else {
        const version = options.deps.getOhMyPiVersion(pi.path);
        add(
            results,
            "pass",
            version ? `Pi ${version} detected at ${pi.path}` : `Pi detected at ${pi.path}`,
        );
        const compare = compareSemver(version, MIN_OH_MY_PI_VERSION);
        if (compare !== null && compare < 0) {
            add(
                results,
                "fail",
                `Pi ${version} is older than required ${MIN_OH_MY_PI_VERSION}. Subagents (historian/dreamer/sidekick) use the long-form \`--extension\` flag introduced in Pi 0.71.0; older versions hard-fail with "Unknown option". Run \`pi update\` (or \`npm install -g @mariozechner/pi-coding-agent@latest\`).`,
            );
        } else if (version) {
            add(results, "pass", `Pi version meets minimum ${MIN_OH_MY_PI_VERSION} requirement`);
        }
    }
    ...
    if (!existsSync(settingsPath)) {
        ...
        if (parsed.error) {
            ...
            if (hasPiPackage(packages)) {
                add(results, "pass", `${OH_MY_PI_PACKAGE_SOURCE} is registered in packages[]`);
            } else {
                add(results, "warn", `${OH_MY_PI_PACKAGE_SOURCE} is missing from packages[]`);
                repairPlan.addPackageEntry = true;
            }
        }
    }
    ...
    if (otherExtensions.length > 0) {
        add(results, "info", `Other Oh My Pi extensions registered: ${otherExtensions.join(", ")}`);
    } else {
        add(results, "info", "No other Oh My Pi extensions listed in settings.json");
    }
    ...
    if (staleCaches.length > 0) {
        ...
        add(
            results,
            "warn",
            `Stale Oh My Pi extension cache found: ${staleCaches.map((entry) => `${entry.path}${entry.version ? ` (v${entry.version})` : ""}`).join(", ")}`,
        );
    } else {
        add(results, "pass", "Oh My Pi extension cache clean (no stale cached package found)");
    }
...
function repair(plan: RepairPlan, prompts: PromptIO): number {
    ...
    if (plan.addPackageEntry) {
        const settingsPath = getOhMyPiUserExtensionsPath();
        try {
            const added = writeOhMyPiSettingsPackage(settingsPath);
            prompts.log.success(
                added
                    ? `Added ${OH_MY_PI_PACKAGE_SOURCE} to ${settingsPath}`
                    : `${OH_MY_PI_PACKAGE_SOURCE} already present in ${settingsPath}`,
            );
            fixed += added ? 1 : 0;
        } catch (error) {
            console.error(
    ...
    if (plan.writeUserConfig) {
        const configPath = getOhMyPiUserConfigPath();
    ...
    for (const path of plan.clearCachePaths) {
        try {
            ...
            prompts.log.success(`Cleared stale Oh My Pi extension cache: ${path}`);
            fixed += 1;
        } catch (error) {
            console.error(
...
async function runIssueFlow(options: {
    ...
    spinner.start("Collecting sanitized Oh My Pi diagnostics");
    try {
