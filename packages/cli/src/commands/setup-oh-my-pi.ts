...
export interface SetupEnvironment {
    detectOhMyPiBinary: typeof detectOhMyPiBinary;
    getOhMyPiVersion: typeof getOhMyPiVersion;
    getAvailableModels: typeof getAvailableModels;
    paths: {
        getOhMyPiAgentConfigDir: typeof getOhMyPiAgentConfigDir;
        getOhMyPiUserConfigPath: typeof getOhMyPiUserConfigPath;
        getOhMyPiUserExtensionsPath: typeof getOhMyPiUserExtensionsPath;
    };
}
...
export function writeOhMyPiSettingsPackage(
    settingsPath: string,
    packageSource = OH_MY_PI_PACKAGE_SOURCE,
): boolean {
...
export async function runSetup(options: RunSetupOptions = {}): Promise<number> {
    ...
    const pi = env.detectOhMyPiBinary();
    if (!pi) {
        spinner.stop("Pi not found");
        prompts.log.warn("Could not find `pi` on PATH or at ~/.omp/bin/pi.");
        prompts.log.message(
    ...
    const version = env.getOhMyPiVersion(pi.path);
    ...
    // the lean subagent extension. Older Oh My Pi versions hard-fail with
    ...
    if (version && comparePiVersion(version, MIN_PI_VERSION) < 0) {
        prompts.log.warn(
            `Pi ${version} is older than the required ${MIN_PI_VERSION}.\n` +
                ...
                `which Pi 0.71.0 introduced. Older Oh My Pi versions hard-fail with ` +
    ...
    const settingsPath = env.paths.getOhMyPiUserExtensionsPath();
    const configPath = env.paths.getOhMyPiUserConfigPath();
    const configurePi = await prompts.confirm("Configure Pi to load Magic Context?", true);
    let packageAdded = false;
    if (configurePi) {
        packageAdded = writeOhMyPiSettingsPackage(settingsPath);
        prompts.log.success(
            packageAdded
