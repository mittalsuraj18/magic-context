/** OhMyPiTestHarness — facade for Oh My Pi Magic Context e2e tests. */
...
import {
    createOhMyPiIsolatedEnv,
    type OhMyPiIsolatedEnv,
    type OhMyPiRunResult,
    spawnOhMyPiTurn,
} from "./oh-my-pi-runner/spawn";

export interface OhMyPiTestHarnessOptions {
...
export class OhMyPiTestHarness {
    readonly mock: MockProvider;
    readonly env: OhMyPiIsolatedEnv;

    private readonly options: OhMyPiTestHarnessOptions;
    private contextDbCached: Database | null = null;
    private turns: OhMyPiRunResult[] = [];

    private constructor(mock: MockProvider, env: OhMyPiIsolatedEnv, options: OhMyPiTestHarnessOptions) {
        this.mock = mock;
        this.env = env;
        this.options = options;
    }

    static async create(options: OhMyPiTestHarnessOptions = {}): Promise<OhMyPiTestHarness> {
        const mock = new MockProvider();
        await mock.start();
        mock.setDefault(options.mockDefault ?? DEFAULT_MOCK_RESPONSE);
        const env = createOhMyPiIsolatedEnv(options.sharedDataDir);
        return new OhMyPiTestHarness(mock, env, options);
    }

    async sendPrompt(
        text: string,
        options: { timeoutMs?: number; continueSession?: boolean } = {},
    ): Promise<OhMyPiRunResult> {
        const { result } = await spawnOhMyPiTurn(text, {
            env: this.env,
            mockProviderURL: this.mockBaseURL(),
            magicContextConfig: this.options.magicContextConfig,
            ompSettingsExtra: this.options.ompSettingsExtra,
    ...
    get lastTurn(): OhMyPiRunResult | null {
    ...
    countTags(sessionId: string, harness = "oh-my-pi"): number {
        try {
    ...
    countPendingOps(sessionId: string, harness = "oh-my-pi"): number {
        try {
            const row = this.contextDb()
    ...
    countDroppedTags(sessionId: string, harness = "oh-my-pi"): number {
        try {
            const row = this.contextDb()
