 ...
 * tables carry a `harness` column ('opencode' or 'oh-my-pi') so per-session
...
const PREFIX = "[magic-context][oh-my-pi]";
...
export default async function (pi: ExtensionAPI): Promise<void> {
	...
	info(
		`loaded v${PLUGIN_VERSION} | harness=oh-my-pi | db=${dbPath} | ` +
	...
	pi.on("before_agent_start", async (event, ctx) => {
		try {
			...
			// Compose the final system prompt: base prompt from oh-my-pi + our
			// magic-context block. oh-my-pi delivers event.systemPrompt as
			// string[] (provider-facing prompt sections). We join, append the
			// block, process for cache stability, then return as a single-
			// element array so the agent sees one coherent system-prompt section.
			const basePrompt = event.systemPrompt.join("\n");
			const composedPrompt = block ? `${basePrompt}\n\n${block}` : basePrompt;
