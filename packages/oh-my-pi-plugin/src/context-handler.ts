...
function collectMessageEntryIds(
	...
	if (ids.length !== expectedLength) {
		...
		log(
			`[magic-context][oh-my-pi]${sessionId ? `[${sessionId}]` : ""} collectMessageEntryIds length mismatch: ` +
...
export function registerPiContextHandler(
	...
	pi.on("context", async (event, ctx) => {
		...
		try {
			...
			if (sessionId === undefined) {
				...
				log(
					"[magic-context][oh-my-pi] context event fired with no session id (falling through unmodified)",
				);
				return;
			}
			...
			log(
				`[magic-context][oh-my-pi] context handler failed (continuing without mutation): ${message}`,
				stack,
			);
	...
	log(
		"[magic-context][oh-my-pi] registered context handler (tagging + drops + nudges)",
	);
}
